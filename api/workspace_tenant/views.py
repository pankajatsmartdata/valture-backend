...
class WorkspaceInviteView(APIView):
    ...
    def post(self, request, workspace_id, *args, **kwargs):
        ...
        if serializer.is_valid():
            ...
            # Generate join URL
            join_url = f"{settings.FRONTEND_URL}/api/workspaces/join/?token={invite.token}"
...
class WorkspaceJoinView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        # Or let the current user join if we allow joining on any logged-in account.
        # Let's enforce that the user's email must match the invite email to prevent joining under wrong accounts.
        ...
        return Response(
            {"message": f"Successfully joined workspace '{invite.workspace.name}'."},
